...
public class Ball extends Ellipse2D.Double {
    ...
    public void setSpeed(double xTrajectory) {
        ...
        // on stocke l'ancienne trajectoire x dans une variable pour savoir le signe
        double oldXTrajectory = this.xTrajectory;
        // pour garder l'angle de la balle, on calcule le rapport entre la trajectoire x et la trajectoire y
        double yBendFactorRatio = yTrajectory / Math.abs(this.xTrajectory);
        this.xTrajectory = Math.abs(xTrajectory);
        // on calcule la trajectoire y en utilisant le rapport defini prealablement
        ...
        if (oldXTrajectory < 0) {
            this.xTrajectory *= -1;  // il faut que la balle avance dans la meme direction qu'avant
        }
    }
    ...
    public void setSpeed(double xTrajectory, boolean shouldRemainAfterServe) {
        ...
        // on change la vitesse de la balle, mais on ne recalcule pas les autres parametres si la nouvelle vitesse
        // ne doit pas rester apres le service.
        // comme on ne change pas la variable currentTrajectory si c'est le cas, la balle reprendra sa vitesse avant
        // le service (voir algorithme de service)
        double oldXTrajectory = this.xTrajectory;
        double yBendFactorRatio = yTrajectory / Math.abs(this.xTrajectory);
        this.xTrajectory = xTrajectory;
        this.yTrajectory = this.xTrajectory * yBendFactorRatio;
    ...
    /**
     * Modifie si la gravite est activee.
     */
    ...
    public void serve() {
        ...
        // la vitesse de la balle. Il faut donc que la balle aie la meme vitesse avant chaque service (c'est a dire quand
        // elle sort du jeu).
        setSpeed(PongCanvas.INITIAL_X_TRAJECTORY, false);

        // on regarde si la coordonnee x de la balle est inferieur a -300 et non 0 pour creer un delai
        if (ballX < -300) {  // la balle est sortie du jeu a droite

            pongCanvas.performOutOfBoundsAction(Paddle.LEFT);
        ...
        // les memes commentaires pour si la balle est sortie du jeu a gauche
        } else if (ballX > pongCanvas.getWidth() + 300) {

            pongCanvas.performOutOfBoundsAction(Paddle.RIGHT);
            scoreSound.playSound();
    ...
    public void serveFromSide(int side) {
        ...
        // Presque le meme algorithme que le service normal
        yTrajectory = 0;
        setSpeed(PongCanvas.INITIAL_X_TRAJECTORY, false);
    ...
    /**
     * Permet de servir la balle depuis le milieu.
     */
    public void serveFromMiddle() {

        // Presque le meme algorithme que le service normal, mais on peut combiner les deux cas (balle sort a gauche/a
        // droite) en deux puisque le service se fera de toute facon depuis le milieu.
        yTrajectory = 0;
        setSpeed(PongCanvas.INITIAL_X_TRAJECTORY, false);
    ...
    public void setGravityCoefficient(double gravityCoefficient) {
        ...
        gravity = Math.abs(xTrajectory) / gravityCoefficient;  // la gravite doit toujours etre positive
    }
    ...
    public void setTerminalMultiplier(double terminalMultiplier) {
        ...
        terminalVelocity = Math.abs(xTrajectory) * terminalMultiplier;  // la vitesse terminale doit toujours etre positive
    }
