...
public class PongWallCollisionDetector implements CollisionDetector {
     ...
     * - la limite inferieure de la balle soit >= la hauteur du jeu
     */
    public boolean ballHitBottom() {

        return ball.getLowerLimit() >= pongCanvas.getHeight();
    }

    /**
     * Methode qui decrit si la balle a frappe le mur superieur. Il faut que:
     * - la limite superieure de la balle soit <= 0
     */
    public boolean ballHitTop() {

        return ball.getUpperLimit() <= 0;
    }

    /**
     * Methode qui decrit si la balle est sortie du jeu. Il faut que:
     * - la limite gauche de la balle soit > la largeur du jeu
     * -------- OU ---------
     * - la limite droite de la balle soit < 0
     */
    public boolean ballOutOfBounds() {

        return ball.getLeftLimit() > pongCanvas.getWidth() || ball.getRightLimit() < 0;
    }
