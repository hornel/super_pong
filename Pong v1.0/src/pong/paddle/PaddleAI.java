...
public class PaddleAI implements ActionListener {
     ...
     */
    protected final static int NO_DIRECTION = 0;
     ...
     */
    protected int getDirectionTowardsBall() {
     ...
     */
    protected int getDirectionTowardsMiddle() {
    ...
    protected void movePaddle() {
        ...
        if (ball.getX() >= pongCanvas.getWidth() / 2 && (paddle.getSide() == Paddle.LEFT ? ball.getXTrajectory() < 0 : ball.getXTrajectory() > 0) && !pongCanvas.isPaused()) {
            ...
            setPaddleSpeed((Math.sqrt((Math.abs(ball.getCenterX() - pongCanvas.getWidth() / 2)) / pongCanvas.getWidth() / 2) * aiDifficulty * ball.getSpeed()));
            paddle.setPaddleY(paddle.getY() + (paddleSpeed * getDirectionTowardsBall()));
    ...
    /**
     * Cherche la vitesse actuelle de l'IA.
     */
